...
from src.domain.model.user import User, UserRole
from src.application.port.output.user_repository import AbstractUserRepository
from src.infrastructure.persistence.models.user import FollowerModel, UserModel
...
class PostgresUserRepository(AbstractUserRepository):
    ...
    async def get_by_display_name(self, display_name: str) -> Optional[User]:
        query_result = await self._session.execute(
            select(UserModel).where(UserModel.display_name == display_name)
        )
        user_record = query_result.scalar_one_or_none()
        return self._to_entity(user_record) if user_record else None
